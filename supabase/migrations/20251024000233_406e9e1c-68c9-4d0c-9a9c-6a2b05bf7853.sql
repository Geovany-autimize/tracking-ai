...
-- Criar função para extrair customer_id do header authorization
...
BEGIN
  ...
  token_jti := current_setting('request.headers', true)::json->>'authorization';
  
  -- Remover "Bearer " do início
  IF token_jti IS NOT NULL THEN
    token_jti := replace(token_jti, 'Bearer ', '');
...
-- Novas políticas para shipment_customers
...
-- Novas políticas para shipments
