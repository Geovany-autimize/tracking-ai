...
export default function BillingPage() {
  ...
  const usagePercentage = totalAvailableInPeriod > 0 ? (totalUsed || 0) / totalAvailableInPeriod * 100 : 0;
  ...
  const plans = [{
    ...
    credits: 5,
    icon: Zap,
    features: ['5 créditos de rastreio/mês', '1 usuário', 'WhatsApp e e-mail básicos', 'Painel unificado', 'Sem SLA'],
    badge: null,
    cta: 'Plano Atual',
    highlight: false,
    tier: 1
    ...
    credits: 1500,
    icon: Crown,
    features: ['1.500 créditos/mês', '3 usuários', 'Regras e jornadas personalizadas', 'Métricas e relatórios', 'Webhooks & API', 'Suporte prioritário'],
    badge: 'Mais popular',
    cta: 'Assinar Premium',
    highlight: true,
    tier: 2
    ...
    credits: null,
    icon: Building2,
    features: ['Volume ilimitado', 'Usuários ilimitados', 'SSO e SAML', 'Infraestrutura dedicada', 'SLA premium 99.9%', 'Account manager', 'Desenvolvimento customizado'],
    badge: null,
    ...
    try {
      if (planId === 'enterprise') {
        ...
        const {
          ...
          body: {
            priceId: 'price_1SMEgFFsSB8n8Az0aSBb70E7'
          }
        });
        if (error) {
  ...
  return <div className="space-y-12 max-w-7xl mx-auto">
      ...
      {subscription?.cancel_at_period_end && plan?.id !== 'free' && <Alert variant="destructive" className="flex items-center gap-4">
          ...
          <Button variant="outline" size="sm" onClick={handleManageSubscription} disabled={isProcessing} className="shrink-0 bg-background hover:bg-background/80">
      ...
      {usagePercentage > 80 && plan?.id !== 'enterprise' && <Alert className="flex items-center gap-4 border-amber-600/50 dark:border-amber-400/50">
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <div className="flex items-center justify-between pb-6 border-b">
            <div className="flex items-center gap-3">
              ...
              <Badge className={`text-sm transition-all duration-200 ${plan?.id === 'free' ? 'bg-muted text-muted-foreground hover:bg-muted/80' : 'bg-primary text-primary-foreground hover:bg-primary/90 hover:scale-105'}`}>
                {plan?.name || 'Free'}
              </Badge>
            ...
            <div className="flex items-center gap-2">
              {plan?.id !== 'free' && <>
                  <Button variant="outline" size="sm" onClick={handleCancelPlan} disabled={isProcessing || subscription?.cancel_at_period_end}>
          ...
          <div className="flex items-center justify-between pb-6 border-b">
            <div>
              ...
              {plan?.id !== 'free' ? <p className="text-sm text-muted-foreground mt-1">
          ...
          <div className="space-y-6">
            {/* Hero Credit Display */}
            {creditsLoading ? <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/10 via-primary/5 to-background border border-primary/20 p-8">
              ...
              </div> : <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/10 via-primary/5 to-background border border-primary/20 p-8">
                <div className="relative z-10">
                  ...
                  <div className="flex items-baseline gap-3 mb-4">
                    ...
                    {extraCredits > 0 && <Badge className="bg-green-500/10 text-green-700 dark:text-green-400 border-green-500/20">
                        +{extraCredits} extras
                      </Badge>}
                  </div>
            ...
            <Collapsible className="space-y-2">
              ...
              <CollapsibleContent className="space-y-3 px-3 pb-3">
                <div className="space-y-2 text-sm">
                  ...
                  {extraCredits > 0 && <div className="flex justify-between">
            ...
            {/* Action Button */}
            <Button variant="outline" onClick={() => setBuyCreditsOpen(true)} disabled={!canBuyExtraCredits} className={cn("w-full relative", !canBuyExtraCredits && "opacity-60")} size="lg">
              <Zap className="w-4 h-4 mr-2" />
              Comprar Créditos Extras
              {!canBuyExtraCredits && <Badge className="ml-2 bg-primary/10 text-primary border-primary/20">
            ...
            {/* Additional Info */}
            {!subscription?.cancel_at_period_end && subscription?.current_period_end && <p className="text-xs text-muted-foreground pt-2">
      ...
      <section className="space-y-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          if (isCurrentPlan) {
            ...
            const cancelDate = format(new Date(subscription.current_period_end), "dd/MM/yyyy", {
              locale: ptBR
            });
            buttonText = `Ativo em ${cancelDate}`;
            buttonDisabled = true;
          }
          return <div key={planItem.id} className={`relative bg-card/50 backdrop-blur-sm rounded-3xl p-8 border transition-all duration-500 hover:shadow-[0_20px_60px_hsl(var(--primary)/0.15)] hover:-translate-y-1 ${planItem.highlight ? "border-primary/50 shadow-[0_20px_60px_hsl(var(--primary)/0.2)]" : "border-border/50 hover:border-primary/30"} ${isCurrentPlan ? 'ring-2 ring-primary' : ''}`}>
                {planItem.badge && <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    {planItem.price === "Consultar" ? <p className="text-4xl font-bold">Sob consulta</p> : <div className="flex items-baseline gap-1">
                  ...
                  <Button variant={planItem.highlight ? "hero" : "outline"} size="lg" className="w-full" disabled={buttonDisabled} onClick={() => handleUpgrade(planItem.id)}>
                    {buttonText}
                  </Button>

                  <div className="space-y-3 pt-4 border-t border-border/50">
                    {planItem.features.map((feature, fIdx) => <div key={fIdx} className="flex items-start gap-3">
